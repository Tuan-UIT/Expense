...
const catColor = (k, list = CATEGORIES) =>
  (list.find((c) => c.key === k) || {}).color || "#8B949E";
const catEmoji = (k, list = CATEGORIES) =>
  (list.find((c) => c.key === k) || {}).emoji || "📦";
...
function addExpense() {
  ...
  renderAll();
  showToast(
    _editingId !== null ? "✅ Đã cập nhật" : "✅ Đã thêm chi tiêu",
    "success",
  );
...
function addIncome() {
  ...
  const amount = parseInt(raw, 10);
  const note =
    document.getElementById("inputIncomeNote").value.trim() || "Thu nhập";
  ...
  if (_editingId !== null) {
    ...
    _editingId = null;
    document.getElementById("incomeSheetTitle").textContent =
      "💚 Thêm thu nhập";
...
function renderSummary() {
  ...
  if (balEl) {
    ...
    balEl.className =
      "amount " + (balance >= 0 ? "balance-pos" : "balance-neg");
  }

  const rate =
    monthIncTotal > 0 ? Math.round((balance / monthIncTotal) * 100) : null;
  setText(
    "savingsRate",
    rate !== null
      ? `Tiết kiệm ${rate}% thu nhập`
...
function renderList() {
  ...
  const filtered =
    activeFilter === "All"
      ? all
      : activeFilter === "__income__"
        ? all.filter((e) => e.kind === "income")
        : all.filter(
            (e) => e.kind === "expense" && e.category === activeFilter,
          );
...
function renderStats() {
  ...
  const savingsRate =
    totalInc > 0 ? Math.round((balance / totalInc) * 100) : null;
  const spendPct =
    totalInc > 0 ? Math.min(100, Math.round((grand / totalInc) * 100)) : null;
  ...
  for (let i = 13; i >= 7; i--) {
    const d = new Date(Date.now() - i * 86400000).toISOString().split("T")[0];
    prev7.push(
      activeExp.filter((e) => e.date === d).reduce((s, e) => s + e.amount, 0),
    );
  }
  const prev7sum = prev7.reduce((a, b) => a + b, 0);
  const trend =
    prev7sum > 0 ? Math.round(((last7sum - prev7sum) / prev7sum) * 100) : null;
  const trendText =
    trend === null
      ? "—"
      : trend > 0
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const prefix = `${qy}-${mm}`;
    const mExp = activeExp
      .filter((e) => e.date.startsWith(prefix))
      .reduce((s, e) => s + e.amount, 0);
    const mInc = activeInc
      .filter((e) => e.date.startsWith(prefix))
      .reduce((s, e) => s + e.amount, 0);
    qMonthRows.push({
      month: qStartMo + i,
  ...
  const isCurrentQuarter =
    qy === qNow.getFullYear() && q === Math.floor(qNow.getMonth() / 3) + 1;
  const qCatTotals = {};
  activeExp
    .filter((e) =>
      qMonthRows.some((r) =>
        e.date.startsWith(`${qy}-${String(r.month).padStart(2, "0")}`),
      ),
    )
    .forEach((e) => {
      qCatTotals[e.category] = (qCatTotals[e.category] || 0) + e.amount;
    });
  ...
  document.getElementById("statsContent").innerHTML = `
    ...
    <div class="stat-block">
      ...
      ${
        !hasQData
          ? '<p style="color:var(--muted);font-size:13px">Chưa có dữ liệu quý này</p>'
          : `
    ...
    <div class="stat-block">
      ...
      ${
        !hasYearData
          ? '<p style="color:var(--muted);font-size:13px">Chưa có dữ liệu năm này</p>'
          : `
      <div class="chart-box" style="height:180px;margin-bottom:12px">
...
function renderStatsCharts({
  ...
  ["chartCategory", "chartLast7", "chartYear", "chartQuarter"].forEach(
    destroyChart,
  );
        ...
        options: {
          ...
          plugins: {
            ...
            tooltip: {
              callbacks: {
                label: (item) =>
                  ` ${item.label}: ${fmtExact(item.raw)} (${grand > 0 ? ((item.raw / grand) * 100).toFixed(0) : 0}%)`,
              },
            },
          },
        },
      ...
      _charts.chartLast7 = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [
            {
              label: "Thu",
              data: last7.map((x) => x.inc),
              backgroundColor: "#3fb950",
              borderRadius: 4,
            },
            {
              label: "Chi",
              data: last7.map((x) => x.exp),
        ...
        options: {
          ...
          plugins: {
            ...
            tooltip: {
              callbacks: {
                label: (item) =>
                  ` ${item.dataset.label}: ${fmtExact(item.raw)}`,
              },
            },
          },
          scales: {
      ...
      _charts.chartQuarter = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [
            {
              label: "Thu",
              data: qMonthRows.map((r) => r.inc),
              backgroundColor: "#3fb950",
              borderRadius: 4,
            },
            {
              label: "Chi",
              data: qMonthRows.map((r) => r.exp),
              backgroundColor: "#f85149",
              borderRadius: 4,
            },
          ],
  ...
  if (hasYearData) {
    ...
    if (ctx) {
      const withData = monthRows.filter(
        (r, i) => i <= monthRows.findLastIndex((x) => x.inc > 0 || x.exp > 0),
      );
...
function showPage(page) {
  ...
  document
    .getElementById("pageStats")
    .classList.toggle("hidden", page !== "stats");
  document
...
function buildCatGrid(gridId, cats, selectFn) {
  document.getElementById(gridId).innerHTML = cats
    .map(
      (c) => `
...
function selectExpCat(el) {
  document
    .querySelectorAll("#catGrid .cat-btn")
...
function selectIncCat(el) {
  document
    .querySelectorAll("#incomeCatGrid .cat-btn")
    .forEach((b) => b.classList.remove("selected"));
  el.classList.add("selected");
}
